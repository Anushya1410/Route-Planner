...
window.addEventListener("load", () => {
    // Fetch weather data for Mumbai by default
    searchByCity('Mumbai');
...
function searchByCity(place = '') {
    var query = place || document.getElementById('input').value;
    var urlsearch = `http://api.openweathermap.org/data/2.5/weather?q=${query}&appid=${apikey}`;
...
function weatherReport(data) {
    ...
    fetch(urlcast).then((res) => {
        ...
        document.getElementById('weathericon.png').src = iconurl;
    });
}
